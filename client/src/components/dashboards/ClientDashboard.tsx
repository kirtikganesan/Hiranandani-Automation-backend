...
const ClientDashboard = () => {
  ...
  useEffect(() => {
    // Fetch client data
    fetch(`${process.env.REACT_APP_BACKEND_URL}/api/client-dashboard`)
  ...
  useEffect(() => {
    // Fetch groups
    fetch(`${process.env.REACT_APP_BACKEND_URL}/api/groups`)
      .then((res) => res.json())
  ...
  const confirmDelete = () => {
    ...
    fetch(`${process.env.REACT_APP_BACKEND_URL}/api/groups/${groupToDelete}`, {
      method: "DELETE",
    })
      .then((response) => response.json())
      .then((data) => {
