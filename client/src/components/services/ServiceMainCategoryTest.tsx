  ...
  const fetchServices = async () => {
    try {
      const [categoriesResponse, countResponse] = await Promise.all([
        axios.get(`${backendUrl}/api/service-main-categories`),
        axios.get(`${backendUrl}/api/service-main-categories-count`),
      ]);

      if (Array.isArray(categoriesResponse.data) && Array.isArray(countResponse.data)) {
        const servicesData = categoriesResponse.data.map((category: any) => {
          const countData = countResponse.data.find(
            (count: any) => count.ServiceMainCategory === category.ServiceMainCategory
          );
          return {
            ...category,
            dependent_services: countData ? countData.dependent_services : 0,
            gst_billing_categories: 0, // Assuming this is static or calculated elsewhere
          };
        });
        setServices(servicesData);
      } else {
        console.error('Unexpected API response:', categoriesResponse.data, countResponse.data);
        setServices([]);
      }
    } catch (error) {
